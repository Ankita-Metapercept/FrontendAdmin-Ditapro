        ...
        methods: {
            ...
            onRoutechange() {
                ...
                setTimeout(() => {
                    ...
                    if (currentPosition > 400) this.$refs.currentMenu.SimpleBar.getScrollElement().scrollTop = currentPosition + 200;
                }, 300);
            },
            isActive(menuItem) {
                    ...
                    <template v-for="item in menuItems">
                        ...
                        <li v-if="!item.isTitle && !item.isLayout" :key="item.id">
                            ...
                            <router-link :to="item.link" v-if="!hasItems(item)" class="side-nav-link-ref">
                                <div class="icon-cust-class" :class="{ active: isActive(item.label) }"><i :class="`bx ${item.icon}`" v-if="item.icon"></i></div>
                                <span v-on:click="setActive(item.label)" :class="{ active: isActive(item.label) }">{{ $t(item.label) }}</span>
                                <span
                                    :class="`badge badge-pill badge-${item.badge.variant} float-right`"
                                    v-if="item.badge"
                                >
